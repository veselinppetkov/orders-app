...
export class ReportsModule {
    ...
    getOrderEurMetrics(order) {
        ...
        const totalEUR = order.totalEUR
            ? CurrencyUtils.normalizeToEUR(order.totalEUR, order.totalBGN ?? order.totalEUR)
            : ((order.costUSD + order.shippingUSD) * order.rate) + extrasEUR;
        const balanceEUR = order.balanceEUR
            ? CurrencyUtils.normalizeToEUR(order.balanceEUR, order.balanceBGN ?? order.balanceEUR)
            : sellEUR - totalEUR;
