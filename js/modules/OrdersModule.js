...
export class OrdersModule {
    ...
    async create(orderData) {
        ...
        try {
            ...
            // Normalize order up-front so Supabase and UI use the same EUR-only payload
            const preparedOrder = await this.prepareOrder(orderData);

            // Create optimistic order for immediate UI feedback
            const optimisticOrder = preparedOrder;
    ...
    async update(orderId, orderData) {
        ...
        try {
            ...
            // Prepare updated order (normalized to EUR)
...
// UNIFIED ASYNC ORDER PREPARATION (used by create, update, duplicate)
    async prepareOrder(data) {
        ...
        // Normalize amounts: accept legacy BGN input but convert to EUR immediately and fix mislabeled EUR values
        const extrasEUR = CurrencyUtils.normalizeToEUR(
            data.extrasEUR,
            data.extrasBGN ?? data.extrasEUR
        );
        const sellEUR = CurrencyUtils.normalizeToEUR(
            data.sellEUR,
            data.sellBGN ?? data.sellEUR
        );
...
// OPTIONAL: Add method to recalculate order with fresh settings
    recalculateOrder(order) {
        ...
        const extrasEUR = CurrencyUtils.normalizeToEUR(order.extrasEUR, order.extrasBGN ?? order.extrasEUR);
        const sellEUR = CurrencyUtils.normalizeToEUR(order.sellEUR, order.sellBGN ?? order.sellEUR);
